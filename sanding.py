...
class SandingEnv(core.Env):
    ...
    def __init__(self, 
                 ...
                 Kp=0.95, Kd=0.5, fix_no_sanding_position=True, fix_robot_initial_position=True):
        self.fix_no_sanding_position=fix_no_sanding_position
        self.fix_robot_initial_position=fix_robot_initial_position
        # Generate n evenly spaced positions along the diagonal
        no_sanding_x = np.linspace(self.MIN_SIZE, self.MAX_SIZE, n_no_sanding+2)[1:-1]  # n evenly spaced points between -50 and 50
        no_sanding_y = np.linspace(self.MIN_SIZE, self.MAX_SIZE, n_no_sanding+2)[1:-1]  # same for y (diagonal points)
        
        self.n_sanding = n_sanding  # Number of sanding circles
        self.n_no_sanding = n_no_sanding  # Number of no-sanding circles

        self.RADIUS = radius
        if self.n_no_sanding==4:
            self.RADIUS=radius*0.8
            no_sanding_x=np.array([-25, -25, 25, 25])
            no_sanding_y=np.array([-25, 25, 25, -25])
            
        elif self.n_no_sanding>4:
            raise ValueError('Number of no-sanding circles should be less than 4')  
        
        # Stack the x and y coordinates into an array of shape (n, 2)
        self.no_sanding_xy = np.column_stack((no_sanding_x, no_sanding_y))
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        def is_valid_position(pos, other_positions):
            for other_pos in other_positions:
                if np.linalg.norm(pos - other_pos) < self.RADIUS * 2:
                    return False
            return True

        # Initialize the state array
        self.state = np.zeros((1 + self.n_sanding + self.n_no_sanding) * 2)

        # Set the robot initial position
        if self.fix_robot_initial_position:
            self.state[:2] = np.array([self.MIN_SIZE + self.RADIUS, self.RADIUS + self.MIN_SIZE])
        else:
            self.state[:2] = self.np_random.uniform(low=self.MIN_SIZE, high=self.MAX_SIZE, size=2)
        # Set the no-sanding positions
        if self.fix_no_sanding_position:
                self.state[-self.n_no_sanding*2:]=self.no_sanding_xy.flatten()
        else:
            self.state[2:2*(self.n_no_sanding+1)] = self.np_random.uniform(low=self.MIN_SIZE, high=self.MAX_SIZE, size=2*self.n_no_sanding)
        # Set the sanding positions and check that they are valid
        for i in range(self.n_sanding):
            while True:
                pos = self.np_random.uniform(low=self.MIN_SIZE, high=self.MAX_SIZE, size=2)
                if i==0:
                    if self.n_no_sanding==4:
                        break
                    else:
                        self.state[2*(i+1):2*(i+2)] = pos
                    break
                if is_valid_position(pos, self.state[:-(self.n_sanding-i)*2].reshape(-1, 2)) and is_valid_position(pos, self.no_sanding_xy):
                    self.state[2*(i+1):2*(i+2)] = pos
                    break
